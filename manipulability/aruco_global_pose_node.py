...
class ArucoGlobalPoseNode(Node):
    def __init__(self):
        super().__init__('aruco_global_pose_node')
        ...
        self.declare_parameter("camera_to_mount_translation", [0.0, 0.0, 0.1])  # 例: x, y, z
        self.declare_parameter("camera_to_mount_rotation_rpy", [0.0, 0.0, 0.0]) # 回転: roll, pitch, yaw [rad]
                ...
                if 0 <= center_y < self.depth_image.shape[0] and 0 <= center_x < self.depth_image.shape[1]:
                    ...
                    try:
                        ...
                        self.get_logger().info(
                            f"Marker ID {ids[i][0]}: ({pose_world.pose.position.x:.2f}, "
                            f"{pose_world.pose.position.y:.2f}, {pose_world.pose.position.z:.2f}) [m]"
                        )
